...
CSRingBuffer::CSRingBuffer() : _RingBufferSize(10000)
	{
...
CSRingBuffer::CSRingBuffer(int bufferSize) : _RingBufferSize(bufferSize)
	{
		InitializeSRWLock(&_Lock);
		_Begin = (char*)malloc(_RingBufferSize);
		_End = _Begin + _RingBufferSize;
...
CSRingBuffer::~CSRingBuffer()
	...
	int CSRingBuffer::Enqueue(const char* pSrc, int size)
	{
	...
	int CSRingBuffer::Dequeue(char* pDest, int size)
	{
	...
	int CSRingBuffer::Peek(char* pDest, int size)
	{
		int useSize = GetUseSize();
		int writePointerPos = GetWritePtrPosition();
		int readPointerPos = GetReadPtrPosition();
	...
	void CSRingBuffer::Lock(bool shared)
	{
		return shared ? AcquireSRWLockShared(&_Lock) : AcquireSRWLockExclusive(&_Lock);
	}

	void CSRingBuffer::Unlock(bool shared)
	{
