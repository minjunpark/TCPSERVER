...
void NetWork()
{
	...
	retSelect = select(0, &rset, NULL, NULL, &times);//반응이 있는 소켓이 있는지 셀렉트로 확인
	...
	if (retSelect > 0)//반응이 있는 리드소켓이 있다면?
	{
...
void RecvProc(Session* session)//받은 메세지처리 프로세스
{
	...
	while (true)
	{
		...
		if(retRecv == SOCKET_ERROR)//소켓 자체에 문제가 생겼다는 뜻이므로
		{
			...
			break;//송신버퍼가 비어있다는 뜻이므로 루프를 탈출한다.
		}
		else//소켓 에러가 아니라면
		{
			//memcpy_s(buffer, sizeof(buffer), buffer, sizeof(buffer));//버퍼 크기만큼 가져오기
...
void sendUniCast(Session* session, char* _Msg)//특정유저에게만 보내기
{
	...
	retUni = send(stmp->sock, _Msg, sizeof(STAR_ID), 0);
	
	if (retUni == SOCKET_ERROR)
	{
		int wsaError;
		wsaError = WSAGetLastError();
			...
			&& wsaError != WSAEWOULDBLOCK)//이에러가 아니라면 로그저장한다.
		{
			char buff[200];
			memset(buff, 0, sizeof(buff));
			sprintf_s(buff, "retRecv Error = %d", WSAGetLastError());
			log_msg(buff);
		}
	}
};
...
void sendBroadCast(Session* session,char* _Msg)//특정 유저만 뺴고 보내기
{
	...
	for (auto session_it = Session_List.begin(); session_it != Session_List.end(); ++session_it)
	{
		...
		retBro = send((*session_it)->sock, _Msg, sizeof(STAR_ID), 0);//세션 소켓으로 바이트전송
		if (retBro == SOCKET_ERROR)
		{
			int wsaError;
			wsaError = WSAGetLastError();
			if (wsaError != WSAEWOULDBLOCK)
