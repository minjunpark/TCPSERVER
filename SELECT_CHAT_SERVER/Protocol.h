...
#define dfROOM_MAX_LEN		512
#define dfPACKET_CODE		0x89
#define dfUSER_MAX_LEN      10000
#define dfPAYLOAD_VALUE     256
#define dfdefaultRoom       0
...
struct Session
{
	...
	DWORD _User_No;//ID값
	DWORD _Connect_Room_No;//접속한 방이름
	WCHAR _Nick_Name[dfNICK_MAX_LEN] ;//유저 이름
	bool _Live;//현재 살아있는 세션인지 아닌지
	bool _Login_check;//현재 살아있는 세션인지
};

struct ROOM
{
	int _Room_No;//방고유 번호
	WCHAR _RoomName[dfROOM_MAX_LEN];//방이름 최대길이 512
	std::list<Session*> _Room_UserList;//현재 방의 유저리스트
};
